...
class AppointmentService:
    ...
    def create_appointment(payload: AppointmentsCreateEdit):
        ...
        # Confirm if patient has pending appointment
        ...
        for key, appointment in appointments.items():
            if patient == appointment.patient and appointment.status == AppointmentsStatus.PENDING:
                raise HTTPException(
                    detail='Patient currently has an appointment in progress. Please finalize the existing appointment before proceeding', status_code=200)
